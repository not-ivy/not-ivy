...
const userRepos:Array<repos> = await githubAPI('/user/repos?per_page=100', 'json');
...
for (const repo of userRepos) {
    if (parse(repo.updated_at, "yyyy-MM-ddTHH:mm:ssZ").getTime() > new Date().getTime() - (1000 * 60 * 60 * 24 * 31) && !repo.fork) {
        activeRepos.push(`- [${repo.owner.login}/${repo.name}](${repo.html_url}) ${repo.description}`);
    }
}
template = template.replace('%starred%', userRepos.slice(0, 5).map(repo => `- [${repo.owner.login}/${repo.name}](${repo.html_url}) ${repo.description}`).join('\n'));
