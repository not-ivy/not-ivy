<!-- deno-fmt-ignore-file -->
<h1 align="center">i-spin</h1>
<div align="center">
  <b><i>Keep it logically awesome.</i></b>
  <br />
  <br />
  <img src="https://img.shields.io/badge/Discord-i--spin-101616?logo=Discord" />
  <a href="https://ch.tetr.io/u/sussyimpostor"><img src="https://img.shields.io/badge/Tetra Channel-sussyimpostor-101616?labelColor=3ebf46&logo=data:image/png;base64," /></a>
  <br />
  <img src="https://img.shields.io/badge/-Vim-%23ffd3da?logo=Vim&labelColor=4c566a" />
  <img src="https://img.shields.io/badge/-CLion-%23ffb4ed?logo=CLion&labelColor=4c566a" />
  <img src="https://img.shields.io/badge/-IntellJ IDEA-%23ffc9e5?logo=IntelliJIDEA&labelColor=4c566a" />
  <img src="https://img.shields.io/badge/-Visual Studio Code-%23ffcee0?logo=VisualStudioCode&labelColor=4c566a" />
  <br />
  <img src="https://img.shields.io/badge/-macOS-%23f4d3d5?logo=macOS&labelColor=4c566a" />
  <img src="https://img.shields.io/badge/-Linux-%23ffbeef?logo=Linux&labelColor=4c566a" />
  <br />
<img src="https://img.shields.io/badge/-JavaScript-ece4db" />
<img src="https://img.shields.io/badge/-Rust-d8e2dc" />
<img src="https://img.shields.io/badge/-other-e8e8e4" />
<img src="https://img.shields.io/badge/-TypeScript-f8edeb" />
<img src="https://img.shields.io/badge/-Shell-ffe5d9" />
<img src="https://img.shields.io/badge/-Kotlin-ffd7ba" />
<img src="https://img.shields.io/badge/-Vue-fec89a" />
<img src="https://img.shields.io/badge/-HTML-fae1dd" />
<img src="https://img.shields.io/badge/-Lua-fcd5ce" />
<img src="https://img.shields.io/badge/-C#-fec5bb" />
  <br />
</div>

## Top 5 Starred Projects

- [i-spin/website](https://github.com/i-spin/website) My new website! ★5
- [i-spin/compass](https://github.com/i-spin/compass) a library for discord webhooks, written using rust ★4
- [i-spin/launch](https://github.com/i-spin/launch) simple and fast browser start/home page ★4
- [i-spin/walls](https://github.com/i-spin/walls) wallpapers i collected (mostly anime) ★3
- [lambda-client-unofficial/lambda-bot-ts](https://github.com/lambda-client-unofficial/lambda-bot-ts) ts bot for the lambda client discord ★3

## Active Projects

- [lambda-client-unofficial/lambda-bot-ts](https://github.com/lambda-client-unofficial/lambda-bot-ts) ts bot for the lambda client discord ★3
- [lambda-client-unofficial/plugin-repo](https://github.com/lambda-client-unofficial/plugin-repo) Unofficial plugin repository ★2
- [lambda-client-unofficial/plugin-repo-api](https://github.com/lambda-client-unofficial/plugin-repo-api) API for the unofficial plugin repository ★0

## Archived Projects

- [i-spin/walls](https://github.com/i-spin/walls) wallpapers i collected (mostly anime) ★3
- [i-spin/launched](https://github.com/i-spin/launched) (WIP) A minecraft launcher. ★2
- [i-spin/mod-updater](https://github.com/i-spin/mod-updater) Easily update your minecraft mods ★2
- [i-spin/OpenTicket](https://github.com/i-spin/OpenTicket) Discord ticket bot written in TypeScript. ★2
- [i-spin/kamiblue-no-watermark](https://github.com/i-spin/kamiblue-no-watermark) kami blue mirror with watermark removed ★1

## Stats

Wakatime stats:
```
JavaScript          [##############################################] 46.74%
Bash                [#############                                ] 13.09%
CSS                 [##########                                   ] 10.13%
TypeScript          [########                                     ] 8.36%
JSON                [######                                       ] 6.26%
HTML                [#####                                        ] 5.46%
Text                [##                                           ] 2.98%
Other               [##                                           ] 2.31%
Git Config          [#                                            ] 1.74%
Properties          [#                                            ] 1.15%
Makefile            [                                             ] 0.58%
YAML                [                                             ] 0.58%
Kotlin              [                                             ] 0.2%
C++                 [                                             ] 0.17%
INI                 [                                             ] 0.15%
Markdown            [                                             ] 0.09%
XML                 [                                             ] 0.01%
```

Most used languages:
```
JavaScript          [##############################] 30%
TypeScript          [#################             ] 17%
Shell               [##########                    ] 10%
Rust                [#####                         ] 5%
Kotlin              [#####                         ] 5%
Vue                 [#####                         ] 5%
HTML                [#####                         ] 5%
Lua                 [##                            ] 2%
C#                  [##                            ] 2%
CSS                 [##                            ] 2%
Python              [##                            ] 2%
```

## Info

This readme will update every 30 minutes through typescript magic! Check back later to see if anything changed!
